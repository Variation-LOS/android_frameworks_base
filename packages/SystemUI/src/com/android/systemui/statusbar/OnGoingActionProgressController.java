...
public class OnGoingActionProgressController implements NotificationListener.NotificationHandler {
     ...
     *
     * @implNote In future this function should be refactored to integrate TunerService to it;
     * @return whether progressbar chip is enabled
     */
    private boolean isEnabled(){
        return context.getResources().getBoolean(R.bool.config_enableOngoingProgressBarChip);
    }
    ...
    private void onNotificationPosted(final StatusBarNotification sbn) {
        if (!isEnabled()) {
            return; /* chip is disabled, nothing to do */
        }
        Notification notification = sbn.getNotification();
