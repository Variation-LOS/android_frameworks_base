...
class UdfpsHelper(
    ...
    private var view = View(context).apply {
    ...
    private suspend fun listenForAnyStateToLockscreenTransition(scope: CoroutineScope): Job {
        return scope.launch {
            transitionInteractor.anyStateToLockscreenTransition.collect {
                if (it.transitionState == TransitionState.STARTED) {
                    view.isVisible = true
                }
            }
        }
    ...
    private suspend fun listenForLockscreenToAnyStateTransition(scope: CoroutineScope): Job {
        return scope.launch {
            transitionInteractor.fromLockscreenTransition.collect {
    ...
    // Since we are not observing KEY_BRIGHTNESS, the display listener updates brightness
    // changes only when the slider is released. So, the dim layer is set to View.GONE when
    // the slider is expected to be utilized.
